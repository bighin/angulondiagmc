...
)
{
    ...
    if (handle==NULL || x==NULL || y1==NULL || y2==NULL || (n<1)) return ;
    ...
    cmd    = (handle->nplots > 0) ? "replot" : "plot";
    title1 = (title1 == NULL) ? "(none)" : title1;
    title2 = (title2 == NULL) ? "(none)" : title2;

    gnuplot_cmd(handle, "%s \"%s\" u 1:2 title \"%s\" with %s,  \"%s\" u 1:3 title \"%s\" with %s", cmd, tmpfname,
                  title1, handle->pstyle, tmpfname, title2, handle->pstyle) ;

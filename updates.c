...
int tau_binary_search_failsafe(struct diagram_t *dgr,double tau)
{
	int c,val;
	
	val=get_nr_midpoints(dgr);
	for(c=0;c<get_nr_midpoints(dgr);c++)
	{
		if(tau<get_midpoint(dgr,c))
		{
			val=c;
			break;
		}
...
void diagram_add_phonon_line(struct diagram_t *dgr,double tau1,double tau2,int lambda,int mu)
{
	...
	*/
	...
	assert(tau_binary_search(dgr,tau1)==tau_binary_search_failsafe(dgr,tau1));
	...
	*/
	...
	assert(tau_binary_search(dgr,tau2)==tau_binary_search_failsafe(dgr,tau2));
